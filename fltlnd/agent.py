...
class DDDQNAgent(Agent):
    ...
    def act(self, obs):
        ...
        if self._eps > np.random.rand(1)[
            0] and self._exploration:  # when training allow random exploration get random action
            action = np.random.choice(self._action_size)
            ...
            action = tf.argmax(action_probs[0]).numpy()
            # obs_predict = obs.reshape((1, 231))
            # action = np.argmax(self.q_eval.predict(action_probs)[0])
        return action
    ...
    def load(self, filename):
        ...
        self.q_target = keras.models.clone_model(self.q_eval)  # Q target model
...
class LSTMAgent(Agent):
    ...
    def create(self):
        ...
        #self._model = (tf.compat.v1.keras.layers.CuDNNLSTM(5, input_shape=(self._state_size,), return_sequences=True))
        # layers.Input(shape=(self._state_size,))
        # shape = (self._state_size,).reshape(-1, 1, 231)
        # layer = tf.compat.v1.keras.layers.CuDNNLSTM(5, input_shape=(-1, 1, 231), return_sequences=True) # Can also use LSTM
        # layer = layers.Dense(128, activation="relu")(layer)
        self._model = Sequential()
        #self._model.add(tf.compat.v1.keras.layers.LSTM(1, input_shape=(self._state_size, ), return_sequences=False))
        self._model.add(tf.compat.v1.keras.layers.LSTM(1, input_shape=(32, 231), stateful=False, return_sequences=False))
